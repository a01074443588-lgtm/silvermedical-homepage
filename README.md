...
- 대표 사진과 본문 사진은 JPG·PNG·WebP 형식의 8MB 이하 파일을 사용합니다. 저장할 때 최대 1600px의 WebP 이미지로 자동 최적화됩니다.
- 대표 사진을 사용하는 경우 접근성을 위한 `사진 설명`을 반드시 입력해야 합니다.
- 게시글 편집 화면은 홈페이지에 표시될 글과 사진을 한 화면에서 직접 다듬는 통합 편집기입니다. 제목 단계, 글꼴·크기, 굵게·기울임·밑줄, 문단 정렬, 목록, 인용문, 링크, 구분선 기능을 제공합니다.
- 본문 사진은 원하는 문단 사이에 추가하고, 선택한 사진을 앞뒤로 이동하거나 사진 설명을 바꾸고 본문에서 제거할 수 있습니다.
- 아래 `본문 사진 보관함`에서는 새 사진을 여러 장 올리고 본문에 삽입하거나 사용하지 않는 파일을 삭제할 수 있습니다. 한 게시글에는 안전장치로 최대 150장까지 등록할 수 있습니다.
- `히스토리`에는 관리자 화면에서 저장한 변경과 블로그 사진 동기화, 일괄 공개·비공개 변경 기록이 남습니다.
...
- 작은 미리보기는 제외하고 네이버 본문의 글, 사진, 구분선을 원문 순서대로 가져옵니다. 첫 사진은 목록과 검색 공유용 대표 사진으로도 저장하고, 본문 안에서는 원래 위치를 그대로 유지합니다.
- 글당 최대 150장의 사진을 처리합니다. 이 제한은 실수로 비정상적으로 많은 파일이 등록되는 것을 막는 안전장치입니다.
- 네이버 사진은 가로 966픽셀 화면용 원본을 요청한 뒤 서버에서 WebP로 최적화합니다.
- 같은 원문 주소의 글이 이미 있으면 기본적으로 제목·본문·게시일·공개 상태는 바꾸지 않고 빠진 사진과 히스토리만 보충합니다.
- 기존 임시저장 글의 본문과 사진을 네이버 원문 순서로 다시 구성하려면 `--refresh-content`를 사용합니다. 공개 상태와 원문 작성일은 유지됩니다.
