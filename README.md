...
- 대표 사진과 추가 사진은 JPG·PNG·WebP 형식의 8MB 이하 파일을 사용합니다. 저장할 때 최대 1600px의 WebP 이미지로 자동 최적화됩니다.
- 대표 사진을 사용하는 경우 접근성을 위한 `사진 설명`을 반드시 입력해야 합니다.
- 게시글 편집 화면에서는 본문 서식 버튼과 홈페이지 모양의 완성 화면 미리보기를 함께 사용할 수 있습니다.
- 추가 사진은 편집 화면 아래에서 사진 설명·표시 문구·순서를 수정하거나 삭제할 수 있습니다.
...
- 작은 미리보기는 제외하고 본문 사진을 원문 순서대로 가져옵니다. 첫 사진은 대표 사진, 나머지는 추가 사진으로 저장하며 글당 최대 40장까지 처리합니다.
- 네이버 사진은 가로 966픽셀 화면용 원본을 요청한 뒤 서버에서 WebP로 최적화합니다.
- 같은 원문 주소의 글이 이미 있으면 제목·본문·게시일·공개 상태는 바꾸지 않고 빠진 사진과 히스토리만 보충합니다.
